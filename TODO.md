...
- [~] **b. Type leaves first, in dependency order:** `vector.py`/`mapping.py` → `rule_model.py` → `devices/` → `adapter`/`aggregator`. Each layer annotates against already-typed dependencies.
  ...
  - [ ] **Next leaf: `util/packet_util.py`.** Now the one remaining untyped dependency of `ip6np_util` (`normalize_*` / `denormalize_*` return `Any`, which is why `bitvector_to_field_value` returns `Any` and `[mypy-util.ip6np_util]` keeps `warn_return_any = False`). Typing it unblocks `warn_return_any` for `[mypy-util.ip6np_util]` — same dividend, next layer.
