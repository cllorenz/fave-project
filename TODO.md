...
### 1h. C++ header-space regression — likely a SOUNDNESS bug (HIGH; needs author + build)
- [ ] **Investigate/fix the header-space merge/equality regression.** (Algorithmic, soundness-critical — to be driven by the author with a C++ build; I can assist with bisect/micro-tests/review, not a blind fix.)
