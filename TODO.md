...
- [ ] **Open (plan §7):** whether the declaration also carries a **group identity** (which would retire the 4 joining sites); the vocabulary beyond `first_match`/`lpm` — **no new term without a consumer**, and `admission` has none until Step 0 lands.
