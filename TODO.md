...
- [~] **b. Type leaves first, in dependency order:** `vector.py`/`mapping.py` → `rule_model.py` → `devices/` → `adapter`/`aggregator`. Each layer annotates against already-typed dependencies.
  ...
  - [ ] **Next: remaining `util/` leaves used by the core slice**, then `devices/`, then `netplumber/{adapter,slice,jsonrpc}.py`. Candidate util leaves (low/no deps): `match_util`, `model_util`, `collections_util`, `tree_util`, `path_util`. Type these bottom-up before the modules that import them.
