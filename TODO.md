...
- [x] **`test_backend_differential` EXTENDED to wl_up and wl_deltanet — 2026-09-22 (CLOUD_BENCH_PLAN.md §2.9).** It was wl_ifi only, which is exactly how APKeep came to over-approximate wl_up silently for months: wl_up's own APKeep tests compare BDD against NDD (which share any approximation) and nothing compared it against a second FAMILY. Now parameterised over three workloads — wl_ifi (forwarding+ACLs), wl_deltanet (in-port-qualified throughout), wl_up (IPv6 routers, packet filters, the in-port-qualified switch defaults that WERE the gap). **All three: the engines agree.** 7 passed, 2 skipped, 56s. **CORRECTION 2026-09-23 — measured, but NOT gated until today.** The file sat in `FAVE_INTEGRATION_TESTS`, which does not export `FAVE_ALLOW_OUT_IFACE=1`, and wl_up's `pgf` carries item 13's `-o` FORWARD rule — so all three `TestBackendDifferentialUp` tests **ERRORED at setup in every integration run** since this workload was added (`./test.sh integration` -> "112 passed, 1 skipped, 3 errors", `RESULT: integration FAILED`). I verified it by hand with the flag exported, which is exactly why the gap stayed invisible: **the number was real, the gate was not**. Moved to `FAVE_OUT_IFACE_TESTS`; wl_ifi and wl_deltanet carry no iptables ruleset so the opt-in is a no-op for them.
  - [x] **It now covers BOTH APKeep engines, which it did not.** The aggregator defaults to `ndd` while this file ran `bdd` only, so the differential validated an engine the benchmarks do not use. Measured on wl_up: same 137-role matrix, both agree with NetPlumber, but **bdd 688.5s vs ndd 4.7s**. (This used to say "the BDD path answers per pair" — wrong, and not discriminating: `adapter.py` queries BOTH engines once per pair. The cause is caching — `lib_ndd` caches the per-source flood and reuses it for every destination, `lib_apkeep` builds a fresh `ReachabilityChecker` per query and caches nothing, which predicts ~137x for 137 roles against a measured 146x. CLOUD_BENCH_PLAN.md §2.12.) Cheap workloads run both; wl_up runs ndd, the engine its published numbers come from.
  - [x] **wl_up asserts agreement only, and that is a property of its ORACLE not of the engines.** Its policy is stateful (3,302 of 18,811 checks carry `f=related:1`), so `reachable.json` records what the policy permits under conntrack while the differential asks an unconditioned all-pairs question. **96 roles differ, identically for both APKeep engines AND NetPlumber** — asserting it would pin a mismatch of questions. `test_apkeep_compliance_cond.py` checks wl_up against its conditioned policy.
  - [x] **`test/gen_wl_deltanet_inputs.sh`** added and wired into the integration tier: wl_deltanet's whole directory is derived from the vendored traces, so a clean checkout has none of it and the workload that exposed the gap would have skipped itself.
- [ ] **Still to extend: wl_i2.** The original note (kept, because the reasoning is unchanged): It is currently `_PREFIX = "bench/wl_ifi"` only, which is how a three-backend "agreement" stood with one backend never compared.
...
- [x] **A REFUSAL CONTRACT for the translation — phase (a) DONE 2026-09-22 (CLOUD_BENCH_PLAN.md §2.7).** Owner's sequencing: (a) first, then (b) ingress demultiplexing with the Stanford subsumption gate. `apkeep/adapter.UntranslatedSemantics`, modelled on `ad6/translate.UnsupportedField`. The contract is NOT "APKeep must express everything" — it is that an approximation must be **declared**: `_ingress_accounted` records per device which mechanism stands in for a semantic the element type cannot carry and how completely. Undeclared → refused; declared-approximate → WARNING so it travels with the result; declared-complete → INFO.
  - [x] **Narrowed twice to be true, and both narrowings are the lesson.** The first form compared every rule's `in_ports` against topology ingress and flagged EVERY ROUTER in the tree, because FaVe's router/packet_filter models key their pipeline stages to INTERNAL ports (`r.routing_in`, `ifi.acl_in_out`) that are not topology ingress. The second still flagged wl_up's packet filters, because `_build_pf_pipeline` subtracts them from `_fwd_devices` locally and runs AFTER the check. Scope is now stated as a decision: discrimination among PHYSICAL ingress ports on devices realised as a `ForwardElement`. Both false positives are pinned by `test/test_apkeep_ingress_contract.py` (9 tests, pure logic, no JVM).
  - [x] **The Stanford/i2 in-stage is declared APPROXIMATE, and checking that claim is what established it.** `_gate_dead_ingress` drops an edge to a port NO rule admits — the dead-interface half. A port admitting a different VLAN set from its neighbour keeps its edge and the element applies every in-stage rule to it: the P7b/P7c over-approximation (240→77). The faithful VLAN paths upgrade it to COMPLETE via per-port ACLElements. Declaring it COMPLETE would have been the exact sin the contract exists to prevent.
  - [x] **NEW FINDING: wl_up has been over-approximating on APKeep all along.** `dmz.uni-potsdam.de` and `wifi.uni-potsdam.de` are SWITCHES (not packet filters), so real `ForwardElement`s, and their default route is in-port-qualified — match-all, lowest priority, out of port 1 (the uplink), from the host-facing ports 2..9 only: "traffic from a host goes up, and does not bounce back down". APKeep drops the restriction, so uplink traffic matching no host route hairpins straight back. Same shape as wl_deltanet's 14 false positives.
  - [x] **Whether it changes a wl_up VERDICT — MEASURED 2026-09-22: it does not any more.** With (b) in place the differential (CLOUD_BENCH_PLAN.md §2.9) shows APKeep and NetPlumber agreeing on wl_up for both APKeep engines. The gap that hid the original unsoundness is closed with it. Superseded note: The existing wl_up APKeep tests compare BDD against NDD — both APKeep engines, both sharing the approximation — and `test_backend_differential` is still `wl_ifi` only, which is what made this invisible. **A wl_up APKeep-vs-NetPlumber differential is what would settle it**, and it is the same open item as extending the differential to wl_i2.
  - [x] **Does `_build_pf_pipeline` carry ingress qualification? YES, and it always did — answered 2026-09-22 (CLOUD_BENCH_PLAN.md §2.11).** By a mechanism the contract does not measure: an `in_port` MATCH field (`-i eth0`) becomes a per-port PREFILTER element `<elem>.inP` that only that port's ingress traverses; `out_port`-qualified rules are dropped as redundant with routing. That IS ingress demultiplexing, arrived at independently for the filter pipeline — so there was nothing to build. `rule.in_ports` and an `in_port` match are different fields and neither mechanism subsumes the other.
    - [x] **Measured, not assumed:** wl_up has 136 filter devices and wl_tum 1; **zero rules in either discriminate among PHYSICAL ingress ports via `rule.in_ports`**, and only `pgf` has more than one physical ingress port at all. 6,554 wl_up chain rules carry `in_ports`, but they name FaVe's internal pipeline ports — the phase-(a) false positive again.
    - [x] **The blanket exclusion is gone**, so a packet_filter's `routing` table is measured like any other device's — a scope statement became a checked claim. Verified NOT load-bearing before removing it: with it deleted the whole APKeep suite passed, the only failure being the unit test asserting the exclusion existed.
    - [x] **`_demux_ingress` now skips filter devices explicitly**, because it re-keys `_fwd_rules`/`_router_fib` but not `_pf_rules`/`_filter_fib` — splitting one would drop its chain rules, the §2.8 wl_up defect one store further along. Such a device falls through to the refusal, which is honest: nothing in the tree needs it, and if something ever does it is refused rather than approximated.
  - [ ] **wl_up and wl_deltanet on APKeep are RED by design until (b).** The gate stated for (a) — "every currently-green workload stays green" — is violated by a TRUE positive, so the gate was wrong as stated. The honest version is "no workload's CORRECT behaviour changes": wl_up+APKeep was never correct, and a refusal is a result rather than a regression.
- [~] **(b) Ingress demultiplexing — BUILT 2026-09-22 (CLOUD_BENCH_PLAN.md §2.8); the gate is PARTIALLY met.** `_demux_ingress` turns a device whose forwarding discriminates among ingress ports into one APKeep element per ingress CLASS (ports grouped by which rules apply there — the coarsest split that loses nothing). A translation, not a model change: the FaVe model keeps its 16 switches, the adapter emits 56 elements. After the split every element is ingress-uniform by construction, so the §2.7 contract passes because nothing is left to account for. **wl_deltanet: all three engines now agree at 0 violations** (APKeep was 14 false positives). Element growth is logged as a real representational cost of the AP model.
  - [x] **`_collapse_out_stage`/`_out_perm` SUBSUMED — proven, not argued.** Disabling the Stanford collapse entirely and rebuilding, `test_reachability_matches_netplumber` still passes: demux splits each out-stage device per ingress port and each class forwards to its own egress, which IS the permutation. Six of the sixteen in-stage routers also genuinely discriminate and are split; 32 devices become 38 elements with reachability unchanged against NetPlumber.
  - [ ] **`_gate_dead_ingress` NOT subsumed, and the reason is informative.** The in-stage discriminates by **VLAN**, and `_translate_fwd_rule` keeps only the destination — so all of an in-stage device's rules translate to the SAME `+ fwd ... 0 0 100000 0` string and demux correctly sees one class. Ingress demultiplexing cannot recover a field dropped before it ran. The §2.7 declaration of those devices as ACCOUNT_APPROXIMATE is therefore accurate rather than a placeholder. **So `self._stanford` and the `in.`/`mid.`/`out.` name coupling CANNOT yet be deleted.**
  - [x] **A naming choice produced a silently wrong answer, and it is recorded.** The first cut named elements `sw.s1|101`; every must-reach check failed (210) while the BDD path was fine and the built model was byte-identical to a working in-process build. The NDD engine keys its per-hop cache as `device + "|" + port` and recovers the device with `indexOf('|')` (`AtomForwarding.java:47`/`:153`, `NddReachabilityEngine.java:89`), so `sw.s1|101` became key `sw.s1|101|151` parsing back as device `sw.s1`. Separator is now `@`, with the reason beside it.
  - [x] **wl_up FIXED 2026-09-22 — the split had to carry BOTH forwarding stores.** The first cut split only `_fwd_rules` and made wl_up WORSE rather than refusing it: its dmz/wifi hold IPv6 host routes in `_router_fib`, keyed by the original device name, so splitting the element left those routes under a name that no longer existed and the run reported violations NetPlumber and ad6 do not — the silent rule-dropping defect committed inside its own fix. Now `_device_tokens` computes the ingress classes over BOTH stores and `_router_fib`/`_ipv6_fib_devices` are re-keyed in lockstep, because neither `ForwardElement` nor `FilterElement` carries an ingress port. **wl_up now passes on APKeep**, including `test_the_conditioned_policy_holds` ("must report no violations, as FaVe+NetPlumber and FaVe+ad6 do"): dmz splits into @1 (uplink) and @2 (host-facing), wifi into @1 and @3 — exactly the "traffic from a host goes up and does not bounce back down" it had been losing.
  - [x] **Scoreboard after (a)+(b):** wl_ifi/wl_cloud/wl_i2/wl_tum unchanged; wl_stanford unchanged vs NetPlumber (32 devices → 38 elements); **wl_up fixed** (silently over-approximating for months); **wl_deltanet agrees with NetPlumber and ad6 at 0 violations**. No workload is red any more.
  - [ ] **wl_up was STILL refused after the first cut (superseded by the entry above).** Demux must not split a device whose forwarding is not fully in its `+ fwd` rules: wl_up's dmz/wifi hold IPv6 host routes in `_router_fib`, keyed by the original name and not rewritten by this pass. Splitting them dropped those routes and produced violations NetPlumber and ad6 do not report — committing the silent-rule-dropping defect inside its own fix. Such devices fall through to the refusal. **Phase (b) does not cover an IPv6 FIB router**, and extending demux to `_router_fib` is what wl_up needs.
  - [x] **Out-stage collapse DELETED 2026-09-22 (CLOUD_BENCH_PLAN.md §2.10).** Checked first what still uses the machinery, and that narrowed the scope: `_collapse_out_stage` was plain-mode only and is gone, but **`_out_perm`/`_capture_out_perm` STAY** because `_build_stanford_faithful` reads them too, and **`self._stanford` STAYS** (it gates the faithful path, `bdd_table` sizing, `_i2_faithful`, and the faithful probe's `vlan=0` target). The `if self._stanford:` block became `if self._stanford and self._faithful_vlan:` — the faithful path keeps the stage because it also models the egress VLAN reset, which demultiplexing does not and should not do. **All 48 wl_stanford switches now survive as 719 elements** (the out stage splits by construction, being a permutation, plus the six in-stage routers), reachability still matches NetPlumber exactly, and the test takes 11s — 719 elements costs nothing measurable. `test_out_stage_collapsed` renamed `test_out_stage_demultiplexed_not_collapsed` and now asserts the opposite of what it did, with a docstring saying so.
  - [ ] **The `in.`/`mid.`/`out.` name coupling still cannot go.** `_gate_dead_ingress` is not subsumed (the in stage discriminates by VLAN, which the translation drops before demux runs), so `self._stanford` is still derived from the presence of a `mid.` stage. Removing that needs the in-stage's VLAN matching to survive translation — a different piece of work.
- [x] **(b) Ingress demultiplexing in the adapter — plan (superseded by the entry above).** One APKeep element per ingress class, emitted by the adapter, invisible to the FaVe model (owner: the benchmark model must NOT be split — 16 devices realistically model the network, and bending it to the weakest backend voids the cross-family comparison). Precedent in the same file: ACLs are already per-port elements. **Gate: it must subsume `_collapse_out_stage`/`_out_perm` and `_gate_dead_ingress` so `self._stanford` and the `in.`/`mid.`/`out.` name coupling can be deleted** — if it cannot retire the special cases it is not the general mechanism. Report the element-count growth as a real representational cost of the AP model rather than absorbing it silently.
...
Owner's framing: *"the sanity question concerning LPM needs to be addressed in future benchmarks."* Emitting rules longest-prefix-first is not evidence that anything would notice if they were not, and this tree has already paid for that once — `_reprioritise_fib_lpm`'s predecessor hardcoded `dev.startswith('mid.')` and so did nothing at all on wl_i2, leaving 3,731 rules shadowed behind a containing prefix while every number computed on them looked fine.

**The check is cheap and mechanical: invert the ordering so the SHORTEST prefix wins, and re-run. If the verdict does not change, the workload's gating evidence is blind to rule priority and needs a dedicated guard.** Two prerequisites, both of which the guard should assert rather than assume: the data must contain a nested prefix pair, and the two prefixes must have different egresses.

- [x] **wl_deltanet — measured, FAILED, guard added.** Inverted, it still reported 256 checks and 0 violations: a switch-granularity reachability matrix asks an existential question per pair, so a misrouted prefix still leaves its 99 siblings arriving. `TestDeltanetLPM` resolves the model's own rules the way NetPlumber does (lowest matching index wins, per in-port) and follows a packet into the one nested pair that witnesses anything — `117.53.131.0/24` homes at s13 while its container `117.53.128.0/20` homes at s2. It asserts both directions, so it cannot pass vacuously. CLOUD_BENCH_PLAN.md §2.5.
- [ ] **wl_cloud — NOT measured.** `cloud_preparation` calls `_reprioritise_fib_lpm` and its own docstring calls it "load-bearing here", but whether the check set would notice an inversion is unknown.
- [ ] **wl_stanford / wl_i2 — NOT measured as check-set sensitivity.** wl_stanford's LPM result is independently validated at 165 pairs and wl_i2's shadowing was the original defect, but that is evidence about the MODEL, not about whether the gating evidence would catch a regression.
- [ ] **Make it a rule for new workloads** rather than a thing remembered: the `bench` tier's verdict gate (item 1s) is the natural place to require it once that gate exists.
...
**Cross-backend (C6) — both other families first got it wrong, and both are FIXED.** **ad6 REFUSED the workload**, correctly and loudly: a field that any rule rewrites must use its integer-valued mutable `fieldmatch` everywhere, and the cloud model both rewrites `ipv4_dst` (NAT) and matches it with 1,668 non-/32 prefixes. That boundary was reached by a workload rather than by argument; closing it (`AD6_PLAN.md` §9.35-§9.37) brought ad6 to 64/64 with NetPlumber and 6/6 on the dataset. **APKeep answered 10 of 64 cells wrong** — dropping every internet-sourced pair and inventing four others — which the soundness gate `bench/apkeep_convergence.py` states in its own docstring forbids outright. **Root-caused and FIXED 2026-09-21, and the first diagnosis was wrong.** That diagnosis read the symptom as a missing topology and blamed the adapter's device-name coupling; dumping the built model refutes it (86 devices, 165 edges, 1,736 rules — the topology was built). The real cause is that `_translate_fwd_rule` turned EVERY forwarding table into a destination-prefix trie. That is exactly right for a FIB, which is all any earlier workload had; wl_cloud is the first whose forwarding tables carry ACLs, so its leaves' "permit one service, then deny the prefix" arrived as a forward and a drop at one priority — and **the two engines behind the one adapter resolved that tie oppositely** (BDD kept the drop, 8 cells reachable; NDD kept the forward, 57), which is as plain a demonstration as one gets that the outcome had stopped being the policy's to decide. Beside it the gateway's 14 DNAT rules arrived with no rewrite and its 5 source-only anti-spoofing drops were skipped. A table now chooses its element from its RULES (`_is_dst_lpm_table`; no device name in the decision): destination-only stays a `ForwardElement`, anything else becomes a first-match `FilterElement` plus a `NATElement` per address rewrite — 46 of 86 devices, 541 filter rules, 28 rewrites. Three things had to be BUILT: a SOURCE rewrite in both engines (`Fields.src_ip` reached `get_field_bdd` and got `BDDFalse`), keyed on the rule's whole 5-tuple because two of the cores' rewrites differ only in `tcp_src`; compliance-check conditions beyond `related` (`protocol`, `port`, negation), the blocker ad6 cleared at §9.37; and — found by fixing those — **the generator seed**, which read the injected source ADDRESS and ignored every other field a generator states, so the matrix phase's `tcp_src` was dropped and 293 denied pairs looked reachable. **APKeep now agrees with NetPlumber on all 64 cells and on every check of all three FPL phases** (oracle 6/6 verdicts + 57 violated; matrix 1,315/4,224; public 3/4,199 — identical violated sets, not merely identical totals). Still open: APKeep's BDD engine **crashes deterministically at ~11 minutes** on the corrected model (item 29) — measured 2026-09-25, superseding both "does not finish in 40 minutes" and its same-day downgrade. A DEFECT and a soundness hazard, not a cost result. `CLOUD_BENCH_PLAN.md` §1.7.3, `fave/test/test_apkeep_{first_match,cloud_differential}.py`.
...
- [x] **Decide: prefix-capable `fieldmatch` in ad6? — DONE 2026-09-21, as a MASK-capable one** (Claas's call: a ternary form subsumes a prefix, so the `<fieldmatch>` path never needs a second mechanism beside it). `AD6_PLAN.md` §9.35. The boundary turned out to be the converter, not the encoding — ad6's other two (`ConvertPortToVariables`, `ConvertCIDRToVariables`) were already prefix-capable, which matters because it changes what the result claims: "a generic model checker cannot do NAT + prefixes" would not have survived a reader opening either one. Measured on wl_cloud: **2,480 of 2,480 address match values translate, 0 refused.**
  ...
  - [x] **APKeep RUNS wl_cloud — DONE 2026-09-21, 64/64 with NetPlumber and identical on every check of all three FPL phases.** Four defects, one shape: a field the model states that the translation did not read. (1) `_translate_fwd_rule` made every forwarding table a destination-prefix trie, dropping every other match field, every header rewrite and the rule ORDER — right for a FIB, and wl_cloud is the first workload whose forwarding tables carry ACLs. A table now picks its element from its rules (`_is_dst_lpm_table`), with no device name in the decision: 46 of 86 devices become first-match `FilterElement`s, 40 keep the trie. (2) An address rewrite had nowhere to go: `NATElement` could write the destination only and keyed on a destination prefix, so a SOURCE NAT — which is what carries wl_cloud's outbound traffic past the gateway's anti-spoofing rules — was inexpressible. Added to both engines, keyed on the rule's whole 5-tuple, because two of the cores' rewrites leave one port with one source /24 and differ only in `tcp_src`. (3) The query path forced only `related`, so the FPL check set's `protocol`/`port`/negated conditions were refused — the blocker ad6 cleared at §9.37. (4) Found by fixing the other three: `add_generator` read the injected source ADDRESS and ignored every other field a generator states, so the matrix phase's per-endpoint `tcp_src` never reached the query and **293 denied pairs looked reachable**. **Measured:** all-pairs 54/64 -> **64/64**; oracle phase 6/6 third-party verdicts and 57 violated checks, matrix 1,315 of 4,224, public 3 of 4,199 — the violated SETS equal NetPlumber's in all three, not just the totals. Mutation-verified (every table a FIB: 9 red; no address rewrite: 6 red; seed narrowed to the address: 1 red; conditions dropped: the oracle phase falls to 4/6 and the public policy reports 817 violations instead of 3). **Toolchain note:** the sandbox had a JRE only, so neither jar could be rebuilt and the committed NDD jar was stale enough that the default engine could not run at all — `openjdk-11-jdk` + `maven` are what `apkeep_smoke.sh`/`ndd_build.sh` already assume. **Still open, and it is not what this said.** Run under the §2.6b protocol with a declared 4 h deadline, APKeep's BDD engine **crashes at ~11 minutes** on the corrected model — reproduced twice, every structural quantity identical. An invalid BDD node handle inside a NAT rewrite, then 36 swallowed `APNotFoundException`s over a half-updated AP partition. See **item 29**; full analysis in `CLOUD_BENCH_PLAN.md` §1.7.3.

  - [x] **ad6 read a rule matching BOTH transport ports as an OR — FIXED 2026-09-22.** Found by running the matrix phase on ad6, which had never been run: the oracle phase agreed 6/6 and the all-pairs matrix agreed on all 64 cells, and the 4,224-check matrix phase reported **1,778 violations against NetPlumber's 1,315 — 463 unexpected, 0 missing**, a pure over-approximation. **Cause:** `KripkeUtils.ConvertToKripke` collected every `<port>` element of a rule into one list regardless of direction and combined them with a DISJUNCTION whenever there was more than one. Right for `--dports 80,443`, which is an alternation; wrong for a source port beside a destination port, which is a conjunction — so `sport=342 AND dport=346` was encoded as `sport=342 OR dport=346`, an over-approximation on ANY rule carrying both. The interface reader immediately above and the VLAN reader immediately below both split by direction; only this one did not. **Fix:** split by direction and mirror the neighbours — alternatives within a direction, a conjunction across them. One change, in the one place both the upstream `InstantiateBase` path and `fave_bridge._instantiate_literal` call. `ad6/FAVE_CHANGES.md` item 32. **My first suspect was wrong and checking beat arguing:** `Instantiator._ShortenPrefixes` really is handed port keys it reads as dotted-quad addresses, which looked conclusive; instrumenting it showed it mutates nothing in either the passing or the failing case. Printing the rule condition settled it in one line. **Why the oracle phase never showed it:** wl_cloud's leaf ACLs match both ports on every rule, so the defect was present there too, but its generators inject no source port — the port is free, a permitted value always exists, and every engine agrees. The matrix generators pin it, and the pinned constraint is the one that went missing. **Blast radius counted, not assumed:** rules carrying `--sport` and `--dport` together number zero in ad6's own `bench/tum`/`bench/up`, zero in wl_tum, wl_up, wl_ifi, wl_example, wl_generic_fw and wl_stanford; wl_shadow has 138,666 and does not reach this code (`BACKENDS_WITH_ANOMALIES` is NetPlumber alone, and no ad6 result for it exists). The fix only narrows an answer, so it cannot invalidate a recorded one. **Result: all three engines now agree on every check of all three phases** — oracle 6/6 verdicts and 57 violated of 71, matrix 1,315 of 4,224, public 3 of 4,199, with the violated SETS equal and not merely the totals. **Guard:** `fave/test/test_ad6_port_pair.py` (7), mutation-verified in BOTH directions — not splitting by direction fails 4, AND-ing same-direction ports fails the alternation test. `CLOUD_BENCH_PLAN.md` §1.7.4, `AD6_PLAN.md` §9.38.
...
- [~] **D1-D5 Delta-net** (CLOUD_BENCH_PLAN.md §2.2). **D1 and D2 DONE 2026-09-22; D3-D5 open.** D1 was the gate — the fourth CSV column had to be established before any converter was written — and it is a **priority encoding longest-prefix match**, `priority = 5*prefix_length + 100`, exact on all 76,200 rows of both traces. It therefore carries no information the prefix does not, so a converter ignores it; `deltanet_trace.parse_trace` asserts the identity on every row so a trace encoding something else in that field is refused rather than misread, and refuses a withdrawal on the same grounds (`*-only-inserts` is a claim about the data, and §2.1's static-first argument rests on it). **D2 collapsed to a census:** no insert overwrites a `(router, prefix)` an earlier one set and no file carries a withdrawal, so replay order cannot matter and the final FIB *is* the file.
  - [x] **Three stated figures were wrong, and the mechanism is the point.** `deltanet-traces/README.md` and CLOUD_BENCH_PLAN.md §2 both carried measurements taken by hand at vendoring time: the fourth field's range (stated 180-220, actually 170-260), the prefix-length span (stated 14-25, actually 14-32), and a prefix count stated of one trace that held for both. Nothing read them, so nothing could contradict them — the same shape as `oracle.json`'s original defect, caught earlier only because D1 forced a measurement. Fixed the way §1.8 requires rather than by retyping: `bench/wl_deltanet/deltanet_census.py` derives them into a generated `TRACES.md`, `test/test_deltanet_census.py` pins it byte for byte (18 tests), and the READMEs now state only what cannot be derived. Had D1 been guessed from the stated range, 9 of the 18 actual priorities would have been outside it.
  - [~] **D3 — the INTERFACE half DONE 2026-09-22 (CLOUD_BENCH_PLAN.md §2.4); the property choice remains.** **And the sub-item this replaces was wrong in a way worth recording.** It read: "a row names neither end's PORT, and FaVe's router model is port-based" — every clause false. `s<i>-<j>` is not a router but a **(switch, port) pair**, which is the paper's own way of modelling ports without a port field: Delta-net splits a switch into one graph node per input port its rules match (§4.1), which is why Table 2 says 68 nodes for a 16-switch network. That count was in the first census and I read it as a name space rather than as a model. **Derived exactly, nothing invented:** 16 switches, 26 undirected links, 68 ports, every switch with `degree + 1` ports — one per neighbour plus port 1, which no inter-switch link ever lands on and which is the external border-router-facing entry point. The **egress port** is the only figure no row carries and it does not need to: the port `i` sends out of towards `k` is the port `i` receives from `k` on, and the switch-level edge set is symmetric. `deltanet_topology.py` REFUSES each invariant rather than assuming it (one link per pair, injective neighbour-to-port, symmetry, ports exactly `1..degree+1`, no intra-switch forwarding, no U-turn) because each is a property of these two files, not of the format. Both traces derive the SAME topology, port map and homing — only the forwarding differs, which is what makes the §2.3 differential clean.
  - [x] **The sinks were not a modelling decision after all.** A node receiving a prefix while carrying no rule for it is where that prefix is DELIVERED. Every one of the 1,400 prefixes terminates at exactly one switch and **14 switches home exactly 100 each** — the paper's "each border router advertises one hundred IP prefixes". So an edge-to-edge property has both ends: traffic enters at port 1 and leaves at its prefix's home switch. This also **accounts for the 1,600-vs-1,400 open question**: s8 and s9 home none and 2x100 is exactly the shortfall. Why they have none is still unknown; they are well connected (degree 7 and 6) but not the best connected, since s2 has degree 8 and homes its hundred.
  - [x] **The property is DECIDED and BUILT — 2026-09-22 (CLOUD_BENCH_PLAN.md §2.5). Owner's call: the reachability matrix as an FPL inventory and policy.** `wl_deltanet` now runs: 16 roles (one per switch, one endpoint each, no services because the data has no transport fields, no `ipv4` because a homing switch stands for 100 prefixes), 119 policy rules stating 210 ordered permissions, **256 checks = 210 must-reach + 46 must-NOT-reach**, and **0 violations on NetPlumber**. Verified NON-VACUOUS rather than asserted: adding `s1 ---> s8` — a destination that homes no prefix — yields exactly one violation naming it. The matrix escapes the all-reachable trap (`AD6_PLAN.md` §5.5 C3: matching an all-reachable oracle is guaranteed for any relaxed encoding) precisely because s8 and s9 home nothing, so 30 off-diagonal cells are denials an over-approximating engine fails. It remains a **consistency property, not an oracle** — the expectation is derived from the same traces as the model.
  - [x] **The policy uses `<-->` where the permission really is symmetric — corrected 2026-09-22 after owner review.** My first version wrote all 210 permissions as `--->`, reasoning that "the two directions are different facts and stating them as one would assert a symmetry nothing here guarantees". Borrowed from wl_cloud's rationale, where it is sound for two reasons that do NOT hold here: wl_cloud's ACLs are stateless (so a symmetric operator would claim something about conntrack) and its 26x26 matrix is genuinely asymmetric cell by cell. This matrix is symmetric wherever both ends are addressable *by construction*, since the policy is what asserts it, and it carries no services at all. `policy_builder` expands a serviceless `<-->` into two unconditional permissions and the compiled CSV is byte-identical — verified. Now 91 `<-->` plus 28 `--->` = 119 rules, and **the mix is worth more than the brevity**: the rules that cannot be written `<-->` are exactly the s8/s9 finding, which 210 identical lines buried. `directed_pairs` asserts the expansion equals the all-`--->` form. `<->>` stays wrong for a reason that does hold — it would condition the return on RELATED,ESTABLISHED.
  - [x] **The paper's own verification goals were DIFFERENT, and that is recorded rather than glossed.** §4.3.1 checks **forwarding loops** on every rule update; §4.3.2 asks **"what is the fate of packets that are using a link that fails"**, one query per link — 158 of them on exactly our snapshot. Pairwise reachability between externally-facing ports is described (Algorithm 3, all-pairs via Floyd–Warshall over atoms) but **never run**, and the paper offers it as pre-deployment testing while citing the Datalog line [17,33] — the NoD family `wl_cloud` already covers. So this workload reproduces the DATA under a property we chose, not the EXPERIMENT. Measured before choosing: loop-freedom is 0 loops and edge-to-edge reachability is 21,000/21,000 — both uniform, hence weak — while the link-failure query gives 100–1,300 affected flows per link. **§4.3.2 is the sharper experiment and the obvious successor.**
  - [x] **LPM now goes through the SHARED `_reprioritise_fib_lpm`, declared as `FIB_TABLE_TYPES = ['sw']` — one stage (owner's call, and the more honest reading of the original benchmark, which models one flat table per node).** Declared as module constants rather than a `config.json`: that file belongs to the raw-table JSON path and this workload's input is two CSVs, so it would be a file nothing reads. The declaration is trivial — it says "all of them" — and that is recorded rather than left to be inferred; wl_cloud's equivalent is load-bearing because it EXCLUDES its NAT gateway, and there is nothing here to exclude. What it buys is one LPM mechanism in the tree rather than two. Shown **inert**: `routes.json` byte-identical to the hand-sorted version. One wrinkle: the function rewrites the INDEX and leaves the list in emission order, which is safe (the adapter passes `rule.idx`, never a list position) but `build_model` sorts by index afterwards so nothing can take position for priority.
  - [x] **LPM is carried by the RULE INDEX, and the matrix cannot check it — found 2026-09-22 on owner review.** FaVe does not reorder a table on its own: `build_model` emits longest-prefix-first, `netplumber/adapter._calc_rule_index` shifts it (`rid << 12`) into NetPlumber where the lower index wins, and nothing downstream repairs a wrong order. `np_preparation._reprioritise_fib_lpm` is not in this workload's path because it emits the order directly rather than loading a raw table in file order. **Verified the ordering is correct AND that the compliance run cannot tell:** inverting it so the shortest prefix wins still gives 256 checks and 0 violations, because the matrix asks an existential question per switch pair and a misrouted prefix still leaves its 99 siblings arriving — the shape of the wl_i2 defect where 3,731 shadowed rules looked fine. Only one of the two nested prefix pairs is even observable (`117.53.131.0/24` homes at s13, its container `117.53.128.0/20` at s2; the other pair shares a home). Guard added as a fast-tier test that resolves the model's rules the way NetPlumber does and follows a packet: correct ordering delivers at s13, inverted at s2, and it asserts both so a walker ignoring the index could not pass. **Argues for §4.3.2 later** — a link-failure property is path-sensitive and would notice; a switch-granularity matrix structurally cannot.
  - [~] **D4 — ALL THREE ENGINES RUN 2026-09-22 (CLOUD_BENCH_PLAN.md §2.6). NetPlumber 0 violations, ad6 0 violations, APKeep 14.** Same 256 checks, same model, one policy; every run logged `completed task check_compliance`. Both zero-verdicts verified NON-VACUOUS by the `s1 ---> s8` mutation, which yields exactly one violation naming it on NetPlumber and on ad6; ad6's 56.9s of solving is itself evidence it did the work (NetPlumber 0.007s, APKeep 1.6s).
  - [x] **APKeep's forwarding translation DROPS `in_ports` — found 2026-09-22, over-approximation, reported not fixed.** All 14 violations are self-pairs (`source.sX reaches probe.sX`) and they are exactly the 14 switches that HAVE a synthesised delivery rule; s8 and s9, which home no prefix and so have none, do not fire. **Two independent confirmations.** (1) By code: `apkeep/adapter._translate_fwd_rule` never reads `rule.in_ports` — its `ForwardElement` is a destination-prefix trie keyed by DEVICE, so a rule meant for certain ingress ports applies to all. Same shape as the four defects in §1.7.3, "a field the model states that the translation did not read". (2) By experiment: rebuilding with delivery rules over EVERY in-port and running NetPlumber reproduces APKeep's 14 violations name for name. **The data never needs the restriction** — of 21,000 (switch, prefix) pairs, 0 forward differently by ingress port, so Delta-net's per-port node split is graph structure, not forwarding. Only the 1,400 SYNTHESISED delivery rules are in-port-qualified.
  - [ ] **Decide how to resolve it (§2.6).** (a) fix APKeep — real but substantial, its ForwardElement is per-device by construction; (b) **recommended** — remodel wl_deltanet with one device per `(switch, port)`, 68 instead of 16, which is literally Delta-net's own model and the count the paper's Table 2 publishes, so no rule needs in-port qualification at all and APKeep's trie becomes exact; (c) record and move on, as C6 originally stood for wl_cloud. (b) removes the disagreement by making the model MORE faithful rather than by weakening a check.
  - [x] **The model: 16 devices, 52 links, 39,500 rules, 16 generators/probes.** The port structure is NOT invented — the contrast with wl_cloud, whose `cloud_preparation` has to warn that its is. The ONE invention is **1,400 synthesised delivery rules**, one per prefix at its home switch: the trace carries inter-switch forwarding only, so a packet arrives at its home and dies and no probe could fire. Derived from the homing, measured to shadow nothing (no prefix has a rule at its own home switch, either trace), and counted separately everywhere so the 38,100 read rules and the 1,400 invented ones are never silently summed.
  - [ ] **D5's premise is weaker than §2.1 assumed.** An insert-only, collision-free trace measures incremental FIB *construction*, never churn. The archive members that would test withdrawal are gone with the archive, so the churn axis needs Claas to re-supply it — worth settling before D4 is costed.
  - [x] **The PAPER arrived mid-task (2026-09-22, supplied by Claas) and it both confirms D1 and corrects the write-up.** Horn, Kheradmand & Prasad, NSDI'17. (1) **D1 confirmed twice over**: §3 states the design ("longest-prefix routing can be simulated by assigning rule priorities according to prefix lengths") and §4.2 states it of this data set (SDN-IP "sets the priority of rules according to the longest prefix match"), so `5*plen+100` is the constant SDN-IP used and D1 stopped being an inference from two files. (2) **`airtel1-only-inserts.csv` IS the paper's published data-plane snapshot** — 38,100 rules, 158 links, 68 nodes, three published figures matching the derivation exactly. That is the **first external check this workload has**: a mis-parse would now be caught from outside the repository. It is NOT an oracle — the paper's results are query TIMES, not sat/unsat — so correctness here stays a consensus between implementations in this tree, and §0's first gap stays wl_cloud's alone. Asserted in `test_deltanet_census.py` against named constants, never blended into the derivation. (3) **The origin URL recorded as "never recorded" is reference [14]**, `https://github.com/delta-net/datasets`. (4) The `s<i>-<j>` names are switch/per-input-port graph nodes — AS 9498 emulated as 16 Open vSwitches, split per matched input port, 16 becoming 68, which is what the D3 "sinks" are. (5) **New open question:** §4.2 says 1,600 unique prefixes; both traces carry 1,400. Recorded, not reconciled.
  - [x] **A differential fell out of it (§2.3), not in the original plan.** The two traces are the same network under two different FAILURE-INJECTION regimes (single-link failures with recovery vs all 2-pair failures; the paper's §4.2, which corrected my first reading of 'routed two ways') — identical prefix, router and next-hop SETS (measured as sets, not inferred from equal counts), differing only in the edge set, 158 vs 155. Three engines agreeing on the DELTA between two models is consensus on a behaviour rather than on an answer, which is what §0's incremental gap actually needs, and both files are already vendored so it costs nothing extra. It does not manufacture an oracle: still a consensus between implementations in this tree.
...
**PLAN: [`OUT_STAGE_PLAN.md`](OUT_STAGE_PLAN.md)** (2026-09-24). **Step 0 is DONE and returned a NEGATIVE RESULT that suspends the rest: deleting all 16 out-stage deny rules changes NetPlumber's OWN answer by nothing** — 165/165 unconditioned, 30/30 under a seed aimed at the TCP deny, 39/39 under one aimed at the anti-spoofing deny — and **165/165 seven times out of seven** when each deny's entire source-controllable header space (six `ipv4_src` values, `ip_proto=6`) is seeded with the destination left free (`bench/apkeep_out_stage_oracle.py --drop-out-denies`). Two independent readings say why: every one of the 68 conditional arrival ports has a *single-egress* catch-all that no narrower permit departs from, so the 1,986 discarded permits are redundant with it and only the 16 denies remove anything; and those denies are written against VLANs 68/78/730/570, none of which is ever reset to 0 *on the device carrying the deny*, while all 16 probes filter `vlan=0` — so denied traffic is transit-only and can never reach a probe. **The collapse is reachability-equivalent to the faithful out stage on wl_stanford, proved structurally and empirically rather than assumed.** The counts below still stand as counts; what changed is their consequence. **Cause (owner, 2026-09-24): the rules are SHADOWED** — a high-priority match-all precedes the whole out-stage ACL, verified upstream in the tracked `stanford-json/152.tf.json` (array order == `id` order == `position` order, match-all lowest on 32 of 32 ports). FaVe preserves that order rather than creating it — and the operator did not write it either: the real `access-list 178` is a well-formed 91-line ACL whose first line is `permit tcp any any established`. The dead-rule structure is introduced by the HSA compilation, which emits the forward ahead of the ACL. **Settled (owner, 2026-09-24): hassel reads tf files top-down, first-match**, so FaVe's reading is correct. Per stage — and only a FIRST-MATCH table can be shadowed this way, never an LPM one, where a leading match-all is just the default route: `in` 0/149 ports shadowed (**live**), `mid` 16/16 but declared LPM (**not shadowing**), `out` 68/68 (**dead**). So wl_stanford's only live filtering is the in-stage VLAN admission, and **2,002 of the out stage's 2,683 rules are unreachable**.

**This does NOT close the item — sec. 7 is revised to proceed, for COST rather than correctness.** A shadowed rule still costs a verification tool, and APKeep is charged **0** of the out stage's 2,683 rules where NetPlumber is charged all 2,683, so `bench/apkeep_vs_netplumber.py` compares two different workloads. Step 3's gate becomes a *reported* cost delta (build time, element count, atom count) with reachability as a regression guard only. Separately: `_capture_out_perm` ignores the match unconditionally, so it would give the same answer if the ACL were live — right answer, wrong reason, and that is the correctness argument that survives.

See OUT_STAGE_PLAN.md sec. 7 for the revised rationale and the two questions that gate it. It also sizes the work: only **68 of 681** out-stage arrival ports carry a condition at all, and all 45 VLAN resets sit on those same 68 — so 613 ports keep the collapse, which is exactly right for them. It also establishes that **reachability cannot move** (on all 68 ports the unioned permutation reaches no more egress ports than the catch-all), which is why the plan's step 0 is an oracle that must FAIL on the current tree.
...
**The same commit flagged the cost, tcp_flags included:** *"reachable.json is fully connected (240/240), an existential role-level oracle … it does not exercise the ingress/egress ACLs, VLAN rewrites, or tcp_flags that this collapse ignores. … Faithful ACL modelling … is deferred to P8."* `APKEEP_BACKEND.md` tracks the deferral as **P7c gap 2 (out-stage / transfer-function fidelity)**, which is still open.

**What is actually discarded** (measured 2026-09-24, wl_stanford):

```
out-stage rules              2,683
  forwarding                 2,667
  forwarding UNDER a match   1,986   <- the condition is discarded
  drops (no action)             16   <- the deny is lost too
  fields: vlan 1986, ip_proto 1624, ipv4_dst 1592,
          tcp_dst 1453, ipv4_src 460, tcp_flags 24
```

`_capture_out_perm` takes `in_port → out_ports` for every forwarding rule and ignores the match, so an **ACL-conditioned permit becomes an unconditional permutation entry** — over-permissive — and 16 explicit denies contribute nothing. Its own docstring says so: *"the egress ACL/VLAN-rewrite are ignored here."* In the PRODUCTION configuration (faithful, the aggregator default) the out stage contributes **zero rules to the engine**: measured `in`=52, `mid`=7,216, `out`=**0**.

**Two things have changed since the trade was made, and they are why this is worth reopening:**

1. **`_demux_ingress` now expresses exactly what the collapse worked around.** The reason for collapsing was that no primitive could carry an in-port-qualified table; demux splits an element per ingress class, and CLOUD_BENCH_PLAN.md §2.10 already used it to delete the **plain**-mode collapse. The faithful path kept its own collapse only because it also models the egress VLAN reset.
2. **`FilterElement` can carry the ACL fields** — proto/src/sport/dst/dport, plus VLAN since item 23's step 0 taught the NDD engine the rule string's slot 17. All but one of the six discarded fields are expressible today.

**The localised symptom is GONE, and that is measured, not assumed.** P7c gap 2 cites a `bbra → rozb` false positive from the collapse. Today both APKeep modes are **pair-identical to NetPlumber** on wl_stanford — 165/165, EXTRA=0, MISSING=0, in faithful *and* plain. So the structural gap currently costs nothing observable on this check set, which is exactly the §2.5 hazard: a reachability matrix can be blind to a dropped constraint, and "reachability is unchanged" does not settle it.

- [ ] **The question worth answering: can the faithful path keep the out stage as a DEMULTIPLEXED device** rather than collapsing it, carrying the ACL, the VLAN reset and the port permutation together? That would close P7c gap 2 and subsume item 25.
- [x] **Sub-item: `packet.upper.tcp.flags` — DONE 2026-09-24** (OUT_STAGE_PLAN.md sec. 4.4). Slot 19 of the `+ filter` layout, carried as a TERNARY pattern in both engines: `ACLRule`/`ConvertTcpFlags` with the field declared LAST so the P6 layout-lock holds, and NDD `FLAGS=9` appended with `withFlagsSlot` at all three readers. `out_stage_rules_widened` 24 → 0; no measurable cost; reachability unchanged (these rules are among the 2,002 the stage shadows). **Original text, for the record:** — 24 rules on 2 devices (`out.yoza_rtr`, `out.yozb_rtr`), all `tcp_flags=1xxxxxxx` + `ip_proto=6` + `vlan=78`. The one discarded field APKeep has no primitive for. Necessary but nowhere near sufficient: adding it changes nothing while the stage is collapsed before any element could carry it. The shape of the fix is known — a trailing token in the `+ filter`/`+ acl` string, parsed in `ACLRule`/`ConvertACLRule` (BDD) and `ruleToNDD`/`withVlanSlot` (NDD) — and both engines must change together or they silently answer different questions (item 23 step 0).
- [ ] **Do not gate any of this on the reachability matrix alone.** A deny-case oracle or a NetPlumber flow-tree differential is needed, per `APKEEP_BACKEND.md`'s own P7c note (*"Do not assert a specific VLAN value until this is decoded"*).
...
**Measured**, in the §7.4 experiment of [`TABLE_SEMANTICS_PLAN.md`](TABLE_SEMANTICS_PLAN.md): removing the stage exemption and carrying the VLAN *match* into each table's `FilterElement` drops wl_stanford from **165 reachable pairs to 150** — *below* NetPlumber, i.e. **unsound**, not merely approximate. Exempting the VLAN-touching devices and letting shape decide the rest reproduces 165 exactly.

**Why:** the in-stage admits a VLAN tag that an upstream mid-stage rewrite assigns. Enforcing the admission table-locally, without modelling the rewrites coherently across the pipeline, yields a model that is more RESTRICTIVE rather than more CORRECT. That whole-pipeline treatment is exactly what `_build_stanford_faithful` does (per-port admission ACLs + inline NATs for the mid rewrites).

- [ ] **Do NOT "fix" `_FILTER_MATCH_FIELDS` by just adding VLAN.** On its own that is the 150-pair model — a soundness regression that **no current test would catch**, because wl_stanford's APKeep test compares against a stored expectation rather than a live NetPlumber run. Any change here must be gated on reproducing 165 pair-for-pair.
- [ ] **A per-table declaration cannot express this**, which is half the reason item 23 closed the role/feature vocabulary question as "no". VLAN here is a property of the pipeline, not of any table in it.
- [x] ~~The VLAN *rewrite* residue needs no new primitive: `+ nat <dev> <port> vlan ...` is simply not wired into `_build_first_match_tables`.~~ **SOLVED differently, 2026-09-25** (OUT_STAGE_PLAN.md sec. 4.5). Wiring the NAT onto the rule's real egress would have been a defect: a NATElement is keyed on (device, port), rewrites everything leaving that port, and has no notion of which filter rule won the first-match race — measured on both engines. Every out-stage rewrite is on a rule sharing the winner's egress, so it would have applied a reset the reference model never performs. **The fix (owner): give the rewriting rule its own egress port, put the NAT there, and wire that port to the same neighbour.** The race is decided at the FilterElement, so only traffic that won via that rule leaves by that port — correct by construction, no analysis, and it carries the *reachable* rewrite case too. The dedicated port supplies the match, so `+ nat ... vlan`'s dst-prefix form is sufficient after all. wl_stanford: 45 rewrites on 45 dedicated ports.
- [ ] Still open from item 23 step 0, same theme: **BDD does not untag probe VLANs and NDD does**, so the adapter's `target_vlan=0` at faithful-wl_stanford probes is a real constraint on one engine and vacuous on the other. Production uses NDD (owner, 2026-09-24), so this is a BDD-path question.
...
**Three artefacts exist only because of it:** `np_preparation._reprioritise_fib_lpm` (LPM *encoded into* first-match order, then repaired at generation time), `apkeep/adapter._is_dst_lpm_table` (the adapter **infers** trie-vs-first-match because nothing tells it), and the twelve `in.`/`mid.`/`out.` name tests in the APKeep adapter (roles inferred from device names, the only channel available). The cost is not hypothetical — `_reprioritise_fib_lpm`'s predecessor hardcoded `dev.startswith('mid.')` and did nothing at all on wl_i2, leaving 3,731 rules shadowed (item 12a, AD6_PLAN.md §5.5).

**Owner's design (2026-09-23):** special semantics are **flagged in the model, not enforced there**; the **adapter implements or rejects** the declared semantics (sorting an LPM table for a positional engine is backend-specific work); the default is **explicit and total** (every table first-match unless overridden) and is **filled in during modelling**, so a benchmark states only its exceptions.

**One proposal of mine was rejected, correctly.** I had the model/aggregator validate that a declared-LPM table *is in* longest-prefix-first order. The owner objected that this contradicts adapter-side implementation — and it does: checking order early is either vacuous or it silently reinstates the positional assumption the design exists to remove. The inverse is the actual payoff: **declaring a table LPM is what frees its order from carrying semantics**, so the vendored raw order survives into the model untouched (better for §1.8's re-derivability, not worse). What survives centrally is *declarability* and *ambiguity*; *implementability* is per-adapter.

**Measured 2026-09-23** (plan §2, all against commit `a6516457`): wl_stanford's declared `mid` FIB has **0** LPM inversions while `in`/`out` have 137 and 4 — the declaration found exactly the right tables, and those 141 are not defects. **Every benchmark is currently LPM-clean** (wl_cloud 86 devices, wl_up 24, wl_ifi 17, wl_deltanet 16, wl_example 3 — zero not-in-order), so this is a **guardrail, not a repair**, and should compete for time on those terms. The ambiguity check finds **nothing anywhere**; the declarability check is already implemented as `_is_dst_lpm_table` (it is what catches wl_cloud's 382 rules on 41 devices). `fib_table_types` is **not name-free** — `r[0].split('.', 1)[0] in set(fib_table_types)` declares which *name prefixes* are FIBs, which is why stageless `wl_deltanet` had to invent a `sw.` prefix to participate.

**Scope, stated so it is not oversold:** the declaration is **intra-table**. Of the twelve APKeep name tests it retires the **5** role-classification sites and leaves the **3** workload sniffs (`self._stanford`) and **4** cross-stage joining sites (`'out.' + router`) untouched. It does not help wl_stanford's in-stage at all, whose VLAN match is dropped by `_translate_fwd_rule` (2,063 rules collapse to 52) — **declared semantics are bounded by what the target primitive can express**.

**Also found while checking whether `FilterElement` could carry VLAN — a live silent divergence (plan §8, not yet filed as its own item):** `_filter_rule_string` already emits a VLAN slot (token 17), always `null`. **BDD honours it** (`FilterElement` → `ACLRule.java:216` → `ConvertACLRule`, including comma-separated sets); **NDD ignores it** (`ruleToNDD` reads tokens 6–15 and 18, never 17; the adjacent `+ acl` branch adds `vlanPred(t[17])`, the `+ filter` branch does not). Measured on wl_tum with the slot forced to `10` and the arrival VLAN pinned: BDD makes vlan20 unreachable, NDD is unchanged. Costs nothing today only because the adapter always writes `null` — but anyone "fixing" the emission gets a correct BDD model and a silently over-permissive NDD one. **Fix is the two lines the `+ acl` branch already has**, plus a differential test; worth doing independently, and a prerequisite for ever declaring `admission`.

- [x] **Step 0 — DONE 2026-09-23.** The NDD engine ignored the `+ filter` VLAN slot (token 17) that the BDD engine has always honoured via `ACLRule`'s P9a field, so one rule string meant two things. Both branches now share `withVlanSlot`; `test/test_ndd_vlan_slot.py` (6 tests, integration tier) **fails on the pre-fix jar and passes on the fixed one**, verified by reverting, rebuilding and re-running. **My first measurement of this was confounded and the plan's §8 table was wrong**: NDD existentially quantifies VLAN out of any `probe.*` device *before* the `target_vlan` constraint, so a test that pins the arrival VLAN at a probe measures the untag, not the slot. The code reading (nothing in the `+ filter` path referenced `t[17]`) is what the fix actually rested on. **Newly open, not part of step 0:** BDD does **not** untag probe VLANs and NDD does — the same intent by different mechanisms, which makes the adapter's `target_vlan=0` at faithful-wl_stanford probes a real constraint on one engine and vacuous on the other. Both gated green today; needs its own measurement before deciding which is right. Also corrected `_first_match_devices`' docstring, which recorded the FilterElement VLAN field as the blocker: it was already half-wrong when written, and what actually blocks is `_translate_fwd_rule` dropping VLAN upstream of any element. **Amended 2026-09-24: step 0 fixed two of the THREE readers of slot 17.** The `+ nat ... match` branch re-heads a FilterElement body and called bare `ruleToNDD`, so an address-rewrite NAT dropped the VLAN on NDD while the BDD side honoured it (`common.ACLRule` token[14]) — the same one-string-two-meanings defect, one reader along. Latent while `_filter_rule_string` always wrote `null`; closed as part of OUT_STAGE_PLAN.md step 2, with a differential measured to fail on a pre-fix jar (`test_ndd_vlan_slot.py::TestVlanSlotInANatMatchBody`).
- [x] **S1+S2 DONE 2026-09-23.** — `AbstractDeviceModel.table_semantics` (overrides only) + `semantics_of()` defaulting to `FIRST_MATCH`, **together with** one producer (`np_preparation` sets `lpm` on wl_stanford `mid` / wl_i2 `out`) and one consumer (`_is_dst_lpm_table` becomes a cross-check that **refuses** on conflict). Landed separately, S1 is unvalidated plumbing; only a producer *and* a consumer prove the declaration survives aggregator dispatch. **Default on read, not materialised**: tables are created lazily (`router.py` `setdefault` at 215/258/294/317) so a dict built in `__init__` would miss them, and 30 test modules read generated model JSON that must stay byte-identical.
  - **The channel was silently LOST at two of its four boundaries**, with the identical symptom each time (every table arriving `first_match`, nothing raising): `_model_from_json` is handed the whole COMMAND so the device sits one level down under `"model"`, and `_sync_diff` hands the engine `model - self.models[node]` — a different object built by one of **four** `__sub__` implementations, none of which carried the field. Both fixed at the **single consumer** rather than the N producers (eight `from_json`, four `__sub__`), which is the transferable lesson. Only a probe of what the ADAPTER received caught either; the serialised JSON was correct throughout.
  - **Result:** wl_stanford `mid`=lpm (16), wl_i2 `out`=lpm (9), everything else first_match — each tracking its OWN `config.json`, which makes the wl_i2 defect structurally impossible rather than merely fixed.
  - **A claim of mine, corrected:** I asserted twice that four device models would drop a base-class field. That was a `def to_json` grep that never checked class membership. All five `AbstractDeviceModel` subclasses carry it; `GeneratorModel`/`ProbeModel` are not subclasses at all. The refusal guard stays but **fires on nothing today**, and says so.
  - **Consumer:** `_is_dst_lpm_table` now cross-checks a declared-`lpm` table against its rules and raises `UntranslatedSemantics` naming the offending field. The converse is deliberately NOT enforced (an undeclared dst-only table still becomes a ForwardElement) — that is S5. `test/test_table_semantics.py`, 16 tests.
- [x] **S4 DONE 2026-09-23 (PROMOTED ahead of the migration)** — declarability + ambiguity checks in the aggregator, with the §4.1 caveat in the docstring (**a passing validation is not evidence a table really is a FIB** — a FIB with a discard aggregate and a deny-before-permit filter have identical shape, which is why this was made a declaration in the first place). They establish that a declaration is coherent *before* anything trusts it to drive reordering.
  - **Both pass everywhere, as §2.6 predicted — a guardrail, not a repair.** Non-vacuous and asserted to stay so: wl_stanford 16 declared tables / 3,844 rules, wl_i2 9 / 77,451, exactly the raw-data counts. Incremental (per-`(node, table)` index carried across batches), so wl_i2's 77k rules replay in 7.0s rather than quadratically.
  - **Declarability is narrower than the adapter's `_LPM_MATCH_FIELDS` on purpose** (no VLAN, no `in_port`): LPM is about which rule WINS, so the neutral question is the match alone. Rewrites are deliberately NOT checked — measured first, because the declared tables carry 3,372 and 77,451 VLAN rewrites and a check rejecting those would reject both declarations.
  - **A discard aggregate is accepted** (`10.0.0.0/8` drop ahead of `10.240.0.0/12` forward) — different prefixes, resolved by LPM. Refusing that shape would block every real FIB, the mistake `_cross_class_promotions` records an earlier design making.
- [x] **S3a DONE 2026-09-23, both backends.** NetPlumber **and ad6** adapters honour `lpm`, with `_reprioritise_fib_lpm` **still running**. Per plan §9.1, `rule.idx` is both priority (`add_rule`'s `r_idx`) and identity (`rule_ids`' key), so this is a *decoupling*, not a move — but the mapping layer already exists (`add_rule` returns the handle `rule_ids` stores). With both active the reordering is idempotent, so **the output must be byte-identical**: a free differential.
  - **Measured on wl_stanford (16 declared tables, 3,844 rules, RPC mocked):** the rule→priority association is **identical**, while the send order differs by construction — `_reprioritise_fib_lpm` rewrites `idx` in place and leaves the list in file order, the adapter sends the sorted list. A second test asserts the send orders DO differ, so the first cannot pass by the adapter doing nothing.
  - **The decoupling was already structural**: `jsonrpc.add_rules_batch` unpacks `_np_rid, t_idx, r_idx, …`, so element 0 is the local identity key and element 2 is NetPlumber's priority. Only their values were both derived from `rule.idx`.
  - **Sort key is `(-prefix length, idx)`**, not a stable sort — stability is correct only while the generation-time repair leaves the list in file order; this key reproduces the generator's assignment after S3b too.
  - **§9.1's incremental question is now a refusal** (`LpmOrderingError`): indices are dense 1..n so a second batch cannot be given a correct priority. Measured: every declared table arrives in exactly one batch today (16/16, 9/9).
  - **ad6 DONE too.** The declaration is threaded to `table_to_ad6` through the adapter's `devices` dict and it sorts by the same `(-plen, idx)` key; all 16 wl_stanford tables order identically either way. A test of mine nearly shipped vacuous — it compared the emitted rule NAMES, which are positional (`r0`, `r1`, assigned after the sort) and identical whatever the order; it now reads the emitted addresses, with a companion test asserting the flag DOES reorder a table whose idx lacks the rank. Previously: `translate.py::table_to_ad6` already sorts by ascending `idx` and its docstring says it depends on `_reprioritise_fib_lpm` having reassigned indices by descending prefix length — so ad6 CONSUMES the repair S3b deletes. It needs the declaration threaded to it and the same sort key.
- [x] **S3b DONE 2026-09-23.** Delete the generation-time reordering, and wl_deltanet's `sw.` prefix / `_DEVICE_PREFIX`. **Gated on S3a green for BOTH positional backends** — if only NetPlumber honours `lpm` when the repair goes away, ad6 silently regresses into the wl_i2 defect. Open risk to decide here: `_calc_rule_index` shifts 12 bits and those slots are reserved for negation expansion, so an incremental insert into an LPM table has no priority headroom without renumbering.
  - **THREE producers, not one.** `np_preparation` (wl_stanford, wl_i2 — both already declared), `deltanet_preparation` (now declares `lpm` on all 16 switches), `cloud_preparation` (declares **nothing**, per the owner decision; its repair was measured a complete no-op — routes byte-identical, 1,741 rules).
  - **Validated:** wl_stanford recomputes **165** pairs on NetPlumber and wl_i2 **61** — the reference figures, with the repair gone. APKeep's 72 on wl_i2 is the over-approximation `test_apkeep_i2` documents by construction, confirmed by a CONTROL on reconstructed pre-S3b inputs giving the same 61 vs 72.
  - **My first NetPlumber harness reported 240 pairs — every pair — which looks exactly like a failed LPM ordering.** It was a harness bug (`_not_reached`: NetPlumber reports node *ids*, APKeep names), caught only by running it against the pre-S3b tree and getting 240 there too. A number that confirms the failure you are looking for deserves a control before it is believed.
  - **Six tests failed and all asserted the ABSENCE of the fix.** The instructive one: `test_inverting_the_priority_breaks_it` inverted the INDEX, which the walk no longer reads, so it would have kept passing while guarding nothing — it now inverts the RESOLUTION. wl_cloud's `TestLpmReprioritisation` is a semantic reversal: it now asserts the index FOLLOWS the dataset, which is what first-match requires.
  - **`_reprioritise_fib_lpm` DELETED 2026-09-23** (owner instruction), with `_cross_class_promotions`, the orphaned `_net_of`/`_forwards`, and their 3 test classes (fast 887→879). `_prefix_len`/`fib_tables` stay — `bench/stanford_priority_check.py` uses them. **That script can no longer fail**: the adapter now orders both of its A/B columns, so its tripwire is a tautology; kept for its narrative with the docstring saying so. Previously recorded as retired-in-place because — nothing calls it, but it is the reference implementation the adapters' `(-plen, idx)` key must agree with, and `_cross_class_promotions` is the only thing here that has ever reported a deny-before-permit swap. Deleting both is a separate decision.
- [x] **S5 DONE 2026-09-23, and half of it was refused.** — `RouterModel` / packet-filter model declare their `routing` table `lpm`. The coverage is not where it looks: **exactly one `RouterModel` exists in the whole suite** (wl_ifi), and wl_up's dst routes sit on 23 switches + 1 packet_filter. Its value is for the *next* benchmark.
  - **The packet-filter half was MEASURED AND REJECTED.** Its `routing` rules select the egress with an `out_port` MATCH — 25 of wl_up `pgf`'s 50 match `out_port` alone, 23 match `out_port` + a destination — so the table is not a destination-prefix trie and declaring it would be false; S4's declarability check would refuse it, correctly. The plan asserted a property of that model that measuring disproved.
  - **Only `RouterModel` declares** (wl_ifi's `ifi.routing`: 9 dst-only rules + one default). One pinned snapshot moved — `test_models.py::TestRouterModel::test_to_json` — and it is the ONLY generated-JSON expectation this whole plan moved, which is what the overrides-only design bought.
- [x] **§7 CLOSED 2026-09-24 by experiment (plan §7.4), and two of three closed as NO.** **Group identity: not needed** — the topology already states the mid↔out relationship (`mid_to_out` is built from the edges 40 lines before `'out.' + router` constructs it; 16/16 agree, all single-valued, none unmapped), so `group` would be a second source of truth for something the model already says. **Vocabulary: `first_match` and `lpm`, nothing else** — `permutation` names an adapter's lossy extraction rather than a meaning (wl_stanford's out stage is a 5-tuple egress ACL *plus* L2 forwarding, and `_capture_out_perm` concedes it ignores the ACL half), and the same stage name means different things across benchmarks (wl_i2's `out` is a pure 77,451-rule dst FIB), which kills role-derivation of any kind. **Roles and features are not needed at all**: letting rule SHAPE decide reproduces NetPlumber's 165 pairs exactly; the rules state the features, and what they cannot state is LPM-vs-first-match — the one declaration built.
  - **The residue turned out to be two things, neither a role**, both filed: **item 24** (`packet.upper.tcp.flags`, the one match field APKeep lacks) and **item 25** (VLAN is cross-stage, so no per-table declaration could express it).
  - [x] **DONE 2026-09-24: the `'out.' + router` name construction is deleted.** Replaced by `out_of_mid`, built from the `mid_to_out` the function already derives from the topology; a mid feeding >1 out stage is now REFUSED rather than silently resolved to one. No-op confirmed by control (faithful wl_stanford: 165 pairs before and after). Three tests, all checked to FAIL on the pre-change adapter — including a grep guard that no adapter constructs a device name, checked precisely because a trivially-passing grep is the vacuous shape this tree keeps hitting.
- [ ] **Gap against §1.8:** none of the plan's figures is currently re-derivable from a committed script. S4's tests are the intended place to close that, since they measure the same properties.
...
### 29. APKeep-BDD corrupts its AP partition on wl_cloud and keeps going — **OPEN, soundness hazard** (2026-09-25)

**Measured, twice, identically.** `bench/cloud_bdd_measure.py --engine bdd` on wl_cloud ends in `status: error` at ~11 min, never approaching its declared 4 h deadline. Byte-identical inputs and jar across both runs; `rules` 1 226/1 773, `ap_num` 53 978, `split_count` 54 053, fatal `APNotFoundException: AP 300498 not exist` — all exact matches. Only wall time moves (672.9 s vs 607.9 s). Artifacts under `fave/bench/wl_cloud/eval/bdd_build_{deadline4h,run2}.*`.

**Two upstream defects, in series.**

1. **`APKeeper.updateSplitAP` is not transactional** (`APKeeper.java:205-241`, upstream `394fe3c7`). It mutates the global `AP` set, then loops every element applying the split, and only **afterwards** calls `ref(parta)`/`ref(partb)`. So the node handles `NATElement.updateAPSplit` passes to `nat()` *inside* that loop are still unreferenced. `nat()` allocates (`exists`, then `and`); a JDD node-table GC triggered by those allocations may collect an unreferenced node. That is the invalid handle: `getVar` returns **-1** and `quant_rec` throws `ArrayIndexOutOfBoundsException: Index -1 out of bounds for length 687` on `varset_vec[var]` (687 = the variable count).

2. **`NATElement.updateRewriteTableIfPresent` swallows it** (`NATElement.java:354`, upstream `aa9822d9`) in a `// TODO Auto-generated catch block`, and `updated` keeps its prior `true`, so the loop retries against the damaged state. 36 cascading `APNotFoundException`s follow before one escapes and ends the run.

**Why it is a soundness hazard, not just a crash.** Nothing on that path stops a build from continuing past the corruption and answering reachability queries over an inconsistent partition. These runs died by which exception happened to escape, not by design. Treat any APKeep-BDD answer on a model whose `NATElement`s participate in AP splits as untrusted until this is resolved.
...
8. Item **23** (model axis, planned in [`TABLE_SEMANTICS_PLAN.md`](TABLE_SEMANTICS_PLAN.md)) — declared table semantics. Design agreed, nothing built. A **guardrail, not a repair** (every benchmark measures LPM-clean today), so it sequences after the gating-validity work; but S1+S2 are small, have no snapshot churn, and prove the channel end-to-end. The §8 NDD/BDD VLAN-slot divergence inside it is a live defect and can be fixed independently, first.
