...
- [~] **b. Type leaves first, in dependency order:** `vector.py`/`mapping.py` → `rule_model.py` → `devices/` → `adapter`/`aggregator`. Each layer annotates against already-typed dependencies.
  ...
  - [ ] **Next: `aggregator/`** (8 files, ~1014 LOC) — the remaining core consumer (`adapter`'s `AbstractVerificationEngine` base lives here, currently under the catch-all). Then the core slice is essentially typed; wire mypy into `lint_test.sh`/CI non-gating → gate (step 6f).
