...
### 1q. Python 3 division semantics: `router.CAPACITY` should be an integer — TODO (confirmed; needs e2e)
- [ ] **Change `RouterModel.CAPACITY = 2**16 / 2**12` to integer division (`//`).** Confirmed with the author: this is a **Python 2 → 3 migration artifact** (item 1) — under py2 `/` between ints was integer division (→ `16`), but py3 makes `/` float division (→ `16.0`). The `# XXX: ugly workaround` comment predates the migration.
- **Effect of the bug:** `aid = int(vlan) * CAPACITY` is a float, so ACL rule indices become floats (e.g. `16.0`) in `RouterModel.persist()`. It is currently *tolerated* — `Rule.from_json` does `int(j["idx"])` on round-trip and `Rule.__eq__` compares numerically — but a freshly-built (not round-tripped) router serialises `idx` as `"16.0"`, and the typing work had to mark that call site `# type: ignore[arg-type]` (`devices/router.py`, the `aid+rid` `Rule(...)`).
- **Fix:** `CAPACITY = 2**16 // 2**12` (value unchanged: `16`), then **remove the `# type: ignore[arg-type]`** on the `aid+rid` `Rule(...)` (mypy's `warn_unused_ignores` will confirm it's no longer needed). Sanity-check the other `idx` arithmetic in `persist()` stays integer.
- **Why not fixed now:** the serialised form changes (`"16.0"` → `"16"`), so it must be validated against NetPlumber's JSON-RPC parsing / stored snapshots under the `e2e`/`bench` tiers before landing. Behaviour was deliberately preserved during the typing pass (see item 6, `devices/` notes). Relates to item 1 (drop Python 2).
...
- [~] **b. Type leaves first, in dependency order:** `vector.py`/`mapping.py` → `rule_model.py` → `devices/` → `adapter`/`aggregator`. Each layer annotates against already-typed dependencies.
  ...
  - **Latent issues flagged for author (behaviour preserved):** (1) `SwitchCommand` del/upd in `switch.main()` passed a bare `Rule` to a `to_json` that iterates `rules` — wrapped in `[...]` (needs e2e check). (2) **`router.CAPACITY = 2**16 / 2**12` is a float**, so ACL rule `idx` becomes e.g. `16.0`; tolerated by `Rule.from_json`'s `int()` + numeric `__eq__` but serialises as `"16.0"`. Left a localized `# type: ignore[arg-type]` to preserve behaviour; proper fix tracked as **item 1q** (confirmed py2→3 artifact: `//` + e2e validation of the wire format). (3) `ProbeModel._normalize_fields(None)` and `aggregator_utils` logger paths now degrade gracefully instead of crashing.
  - [ ] **Next: `netplumber/{jsonrpc,slice,adapter}.py`** then `aggregator/`. `adapter.py` is the large NetPlumber RPC adapter — the last big consumer. Commit per module; fast tier green throughout.
