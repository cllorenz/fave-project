...
- [x] **Decide: prefix-capable `fieldmatch` in ad6? — DONE 2026-09-21, as a MASK-capable one** (Claas's call: a ternary form subsumes a prefix, so the `<fieldmatch>` path never needs a second mechanism beside it). `AD6_PLAN.md` §9.35. The boundary turned out to be the converter, not the encoding — ad6's other two (`ConvertPortToVariables`, `ConvertCIDRToVariables`) were already prefix-capable, which matters because it changes what the result claims: "a generic model checker cannot do NAT + prefixes" would not have survived a reader opening either one. Measured on wl_cloud: **2,480 of 2,480 address match values translate, 0 refused.**
  ...
  - [ ] **ad6 loses BOTH port matches when one rule carries both — OPEN, found 2026-09-22 by running the matrix phase.** The oracle phase agrees 6/6 and the all-pairs matrix agrees on all 64 cells, so the workload looked settled; the 4,224-check matrix phase reports **1,778 violations against NetPlumber's 1,315 — 463 unexpected, 0 missing**, a pure over-approximation. **Minimal reproducer** (`fave/test/test_ad6_port_pair.py`, `expectedFailure` so a fix reports an unexpected success rather than passing unnoticed): two switches in series permitting different SOURCE ports is answered correctly, and adding a destination port to both rules — the SAME one, discriminating nothing — makes the conflicting source ports stop conflicting. Symmetric: conflicting destination ports bind until a source port is added beside them. Either port alone is honoured, the two together are both lost. **Why the matrix phase and not the oracle phase:** wl_cloud's leaf ACLs match both ports on every rule, so the defect is present in both, but the oracle generators inject no source port — the port is free, a permitted value always exists, and every engine agrees. The matrix generators pin it, and that is the constraint that goes missing. **NOT the generator seed**, which was the guess by analogy with APKeep's fourth defect: `translate.generator_device` already carries the whole injected header (immutable as a match, mutable as a rewrite on the injection edge, §5.4 B2). **Suspected, part-demonstrated, not proven:** `Instantiator._ShortenPrefixes` is the IP-prefix optimisation and its caller feeds it every `src_`/`dst_`-prefixed key, so `src_port_342` arrives there, is read as a dotted-quad with an implicit /32, canonises to a nine-bit string, and its rewriting step strips `Conjunction[:32]` from a sixteen-variable port equality. That port keys reach it and are mangled is checkable in a REPL; that this is the path taken here is not yet shown. Fixing it is ad6-encoding work, the same territory as §9.35-§9.37. `CLOUD_BENCH_PLAN.md` §1.7.4.
