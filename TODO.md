...
- [~] **b. Type leaves first, in dependency order:** `vector.py`/`mapping.py` → `rule_model.py` → `devices/` → `adapter`/`aggregator`. Each layer annotates against already-typed dependencies.
  ...
  - [ ] **Next: `util/path_util.py`** (used by `netplumber/slice.py`). More involved: fall-through `None` returns (`str_to_pathlet`, `pathlet_to_str`) need consumer narrowing, and `json_to_pathlet` is a JSON/`Any` boundary (likely `warn_return_any = False`). Then `devices/`, then `netplumber/{adapter,slice,jsonrpc}.py`.
