...
- [ ] **Verify on CI:** if `pandoc` errors on a missing `.sty` (likely culprits: `bookmark.sty`, `parskip.sty` from `texlive-latex-extra`), add `texlive-latex-extra`. Non-gating (e2e), so a failed attempt just shows in logs.
