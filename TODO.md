...
- [ ] **S5 (last)** — `RouterModel` / packet-filter model declare their `routing` table `lpm`. The coverage is not where it looks: **exactly one `RouterModel` exists in the whole suite** (wl_ifi), and wl_up's dst routes sit on 23 switches + 1 packet_filter. Its value is for the *next* benchmark.
