...
- [x] ~~The VLAN *rewrite* residue needs no new primitive: `+ nat <dev> <port> vlan ...` is simply not wired into `_build_first_match_tables`.~~ **WRONG, and answered 2026-09-25** (OUT_STAGE_PLAN.md sec. 4.5). Wiring it in would have been a defect: a NATElement is keyed on (device, port) and applies to everything leaving that port, with no notion of which filter rule won the first-match race — measured on both engines. And every out-stage rewrite is on a *shadowed* rule sharing the winner's egress, so emitting it would apply a reset the reference model never performs (item 27's defect, newly introduced). `_shadowed()` now proves such rewrites dead and drops them (wl_stanford 45, wl_cloud 3), and refuses a rewrite that could actually fire. No workload has a VLAN rewrite on a first-match table anyway: wl_ifi's 10 are on a dst-LPM FIB.
