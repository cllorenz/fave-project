...
- [ ] **S1** — `AbstractDeviceModel.table_semantics` (overrides only) + `semantics_of()` defaulting to `FIRST_MATCH`; `to_json` emits only when non-empty. **Default on read, not materialised**: tables are created lazily (`router.py` `setdefault` at 215/258/294/317) so a dict built in `__init__` would miss them, and 30 test modules read generated model JSON that must stay byte-identical.
- [ ] **S2** — one producer (`np_preparation` sets `lpm` on wl_stanford `mid` / wl_i2 `out`, replacing the name-prefix match) and one consumer (`_is_dst_lpm_table` becomes a cross-check that **refuses** on conflict). Proves the channel survives aggregator dispatch.
- [ ] **S3** — move `_reprioritise_fib_lpm` from generation time into the NetPlumber and ad6 adapters; delete wl_deltanet's `sw.` prefix.
- [ ] **S4** — declarability + ambiguity checks in the aggregator, with the §4.1 caveat in the docstring (**a passing validation is not evidence a table really is a FIB** — a FIB with a discard aggregate and a deny-before-permit filter have identical shape, which is why this was made a declaration in the first place).
- [ ] **S5** — `RouterModel` / packet-filter model declare their `routing` table `lpm`. NB the coverage is not where it looks: **exactly one `RouterModel` exists in the whole suite** (wl_ifi), and wl_up's dst routes sit on 23 switches + 1 packet_filter.
- [ ] **Open (plan §7):** whether the declaration also carries a **group identity** (which would retire the 4 joining sites); the vocabulary beyond `first_match`/`lpm`; who writes it for rich models.
