...
- [~] **b. Type leaves first, in dependency order:** `vector.py`/`mapping.py` → `rule_model.py` → `devices/` → `adapter`/`aggregator`. Each layer annotates against already-typed dependencies.
  ...
  - [ ] **Next leaf: `util/ip6np_util.py`.** It is the one remaining untyped dependency of `rule_model.py` (`field_value_to_bitvector` / `bitvector_to_field_value` return `Any`). Typing it unblocks turning `warn_return_any` ON for `[mypy-rule.*]` — concrete payoff of the leaf-first order.
