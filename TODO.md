...
- [~] **b. Type leaves first, in dependency order:** `vector.py`/`mapping.py` → `rule_model.py` → `devices/` → `adapter`/`aggregator`. Each layer annotates against already-typed dependencies.
  ...
  - [ ] **Next: consumer modules.** Remaining core-slice util deps used by `devices/`/`netplumber/`: `aggregator_utils`, `lock_util` (type as leaves first). Then `devices/*.py`, then `netplumber/{adapter,slice,jsonrpc}.py`. These are larger consumers and will surface more type decisions — proceed module-by-module, commit per module.
