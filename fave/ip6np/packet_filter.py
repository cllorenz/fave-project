...
class PacketFilterModel(Model):
        def __init__(self,node):
            ...
            self.ports = {
                "pre_routing_in"        : 1,
                "pre_routing_input"     : 2,
                "pre_routing_forward"   : 3,
                "input_states_in"       : 4,
                "input_states_accept"   : 5,
                "input_states_miss"     : 6,
                "input_rules_in"        : 7,
                "input_rules_accept"    : 8,
                "forward_states_in"     : 9,
                "forward_states_accept" : 10,
                "forward_states_miss"   : 11,
                "forward_rules_in"      : 12,
                "forward_rules_accept"  : 13,
        ...
        def to_json(self):
            ...
            chain_to_json = lambda k: \
                [(i,r.vector.vector,r.action) for i,r in enumerate(self.chains[k])]

            self.tables = {
                k:chain_to_json(k) for k in self.chains if k in active
            }
            self.ports = {
        ...
        def __str__(self):
            return "%s\nrules:\n\t%s\nchains:\n\t%s\nports:\n\t%s" % (
                str(super(PacketFilterModel,self)),
                str(self.rules),
                str(self.chains),
                str(self.ports)
            )
