...
#    ]))
    ...
    inventory["vlan_to_domain"] = {
        get_vlan(*v) : d for d, v in list(mapping.items())
    }
    inventory["domain_to_vlan"] = {
        d : get_vlan(*v) for d, v in list(mapping.items())
    }
    inventory["domain_to_ip"] = {
        d : get_ip(*v) for d, v in list(mapping.items()) if has_ip(*v)
    }
    inventory["ip_to_domain"] = {
        get_ip(*v) : d for d, v in list(mapping.items()) if has_ip(*v)
    }
