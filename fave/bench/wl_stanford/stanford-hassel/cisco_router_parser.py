...
class cisco_router(object):
  ...
  def parse_interface_config(self,iface_info,file_path):
    ...
    if iface.startswith("vlan"):
      ...
      if len(parts) > 1:
        ...
        vlan = int(parts[1])
        iface = parts[0]
        if str(vlan) not in list(self.configed_vlans.keys()):
    ...
    for (line,line_counter) in iface_info:
      if line.startswith("shutdown"):
        ...
        tokens = line.split()
        if not tokens[2] in list(self.acl_iface.keys()):
    ...
    if shutdown:
      ...
      for v in list(self.configed_vlans.keys()):
      ...
      if is_ip_subnet(tokens[0]):
        ...
        if len(tokens) > 2:
          ...
          if port.lower().startswith("vlan") and \
            ...
            if (tokens[1] in list(self.arp_table.keys())):
              ...
              if vln.startswith("vlan"):
                ...
                if vm_key in list(self.mac_table.keys()):
          ...
          # next hop is an attached vlan  
            ...
            parts = re.split('\.',port)
            if len(parts) > 1 and self.replaced_vlan != None and int(parts[1]) == self.replaced_vlan: #self.replaced_vlan[0]:
  ...
  def generate_port_ids(self, additional_ports):
    ...
    for elem in self.config_ports:
      s.add(elem)
    for vlan in list(self.vlan_span_ports.keys()):
  ...
  def generate_port_ids_only_for_output_ports(self):
    ...
    for fwd_rule in self.fwd_table:
      ...
      if len(m) > 1:
        ...
        if fwd_rule[2] in list(self.vlan_span_ports.keys()):
  ...
  def generate_transfer_function(self, tf): 
    ...
    for cnf_vlan in list(self.configed_vlans.keys()):
      if "vlan%s"%cnf_vlan not in list(self.vlan_span_ports.keys()):
        continue
      trunk_ports = []
      access_ports = [] 
    ...
    if self.def_vlan in list(self.configed_vlans.keys()):
      for port in list(self.port_to_id.keys()):
        if port != "self" and port not in all_access_ports:
          port_id = self.port_to_id[port]
          match = wildcard_create_bit_repeat(self.hs_format["length"],0x3)
        ...
        if fwd_rule[1] == subnet:
          ...
          # non drop rules
            ...
            # physical ports - no vlan taging
              ...
              if fwd_rule[2] in list(self.port_to_id.keys()):
    ...
    for subnet in range(32,-1,-1):
      for fwd_rule in self.fwd_table:
        if fwd_rule[1] == subnet:
          ...
          self.set_field(match, "ip_dst", int(fwd_rule[0]), 32-subnet)
          in_ports = []
          for p in list(self.port_to_id.keys()):          
          ...
          # non drop rules
            ...
            # vlan outputs
              ...
              if fwd_rule[2] in list(self.vlan_span_ports.keys()):
                port_list = self.vlan_span_ports[fwd_rule[2]]
                for p in port_list:
                  if p in list(self.port_to_id.keys()):
                    out_ports.append(self.port_to_id[p]+ \
                                 self.PORT_TYPE_MULTIPLIER * \
                                 self.OUTPUT_PORT_TYPE_CONST)
              else:
...
if __name__ == "__main__":
  for idx, prefix_id in enumerate([
    ...
    cs.read_arp_table_file("%s_arp_table.txt" % prefix_id)
    cs.read_mac_table_file("%s_mac_table.txt" % prefix_id)
    cs.read_config_file("%s_config.txt" % prefix_id)
    cs.read_spanning_tree_file("%s_spanning_tree.txt" % prefix_id)
    cs.read_route_file("%s_route.txt" % prefix_id)
    cs.optimize_forwarding_table()
    cs.generate_port_ids([])
    cs.generate_transfer_function(tf)
    tf.save_object_to_file("../stanford-tfs/%s.tf" % prefix_id)
