...
if __name__ == '__main__':
    ...
    routes = [
        ("pgf", 1, 1, ["ipv6_dst=2001:db8:abc:0::0/64"], ["fd=pgf.2"]),
        ("pgf", 1, 1, ["ipv6_dst=2001:db8:abc:1::0/64"], ["fd=pgf.3"]),
        ("pgf", 1, 65535, [], ["fd=pgf.1"]),
        ("dmz", 1, 65535, [], ["fd=dmz.1"]),
        ("wifi", 1, 1, ["ipv6_dst=2001:db8:abc:1::0/64"], ["fd=wifi.2"]),
    ...
    for cnt, host in enumerate(hosts):
        ...
        routes.extend([
            ("dmz", 1, 1, ["ipv6_dst=%s" % addr], ["fd=dmz.%s" % (cnt+2)]),
    ...
    for cnt, subnet in enumerate(subnets):
        ...
        routes.append((
            ...
            1,
            port,
            ["ipv6_dst=2001:db8:abc:%s::0/64" % port],
            ["fd=pgf.%s" % port]
        ))
        ...
        for srv, host in enumerate(subhosts):
            ...
            ident = srv + 1
            sport = srv + 2
            addr = "2001:db8:abc:%s::%s" % (port, ident)
            ...
            routes.append((hostnet, 1, 1, [], ["fd=%s.%s" % (hostnet, 1)]))
