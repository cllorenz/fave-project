...
#            (3,[31],[33],"xxxx1001xxxxxx11","x"*16,"xxxx1000xxxxxx11"),
    ...
    nodes = prepare_network(sock,tables,links,sources,[probe])

    # results in false probe condition
    remove_rule(sock,nodes[2][2])
...
def cycle_test(sock):
    ...
    nodes = prepare_network(sock,tables,links,sources,probes)

    # add new rule to close the cycle
    add_rule(sock,*(2,1,[3],[4],"x"*8,"x"*8,None))

    reset_plumbing_network(sock)


def slicing_test(sock):
    tables = [
        # (t_idx,t_ports,[(r_idx,[in_ports],[out_ports],match,mask,rw)])
    ...
    add_rule(sock,*(1,2,[1],[2],"xxxxx111","x"*8,None))

    # setup slice
    add_slice(sock,1,*(["xxxxx1x1"],None))

    # add slice to introduce overlap
    add_slice(sock,2,*(["xxxxx111"],None))
    remove_slice(sock,2)

    result = add_rule(sock,*(1,3,[1],[2],"xxxxxx1x","x"*8,None))
    remove_rule(sock,result)

    # add rule to introduce leakage
    result = add_rule(sock,*(2,2,[3],[4],"xxxxx111","x"*8,"xxxxx110"))
    remove_rule(sock,result)
...
def policy_test(sock):
    ...
    add_policy_probe(sock,[7])
    add_link(sock,6,7)
    add_link(sock,99,1)

    # add policy rules
    add_policy_rule(sock,2,{"list":["xxxxxx11"],"diff":None},"allow")
    add_policy_rule(sock,2,{"list":["x"*8],"diff":None},"deny")

    # add uncritical rule
    add_rule(sock,*(2,1,[3],[4],"xxxxx111","x"*8,None))

    # add critical rule
    add_rule(sock,*(2,2,[3],[4],"xxxxxx11","x"*8,"xxxxxx10"))
