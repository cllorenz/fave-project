...
echo -n "create hosts (source) in dmz... "
cnt=2
for  HOST in $HOSTS; do
    ...
    PYTHONPATH=. $TIME -ao $SRCLOG python2 topology/topology.py -a -t generator -n $H -f "ipv6_src=$A"
    PYTHONPATH=. $TIME -ao $SRCLLOG python2 topology/topology.py -a -l $H.1:dmz.$cnt

    cnt=$(($cnt+1))
done
echo "ok"

echo "create hosts (source) in subnets..."
cnt=4
for NET in $SUBNETS; do
    ...
    for HOST in $SUBHOSTS; do
        port=$(($srv+1))
        SRV=source.`echo $HOST | cut -d ',' -f 1`.$NET
        ADDR=2001:db8:abc:$cnt::$srv
        PORTS=`echo $HOST | cut -d ',' -f 2-`

        # create source generator
        PYTHONPATH=. $TIME -ao $SRCLOG python2 topology/topology.py -at generator -n $SRV -f "ipv6_src=$ADDR"

        # link generator to switch
        PYTHONPATH=. $TIME -ao $SRCLLOG python2 topology/topology.py -al $SRV.1:$NET.$port
    done
...
for NET in $SUBNETS; do
    ...
    for HOST in $SUBHOSTS; do
        port=$(($srv+1))
        SRV=probe.`echo $HOST | cut -d ',' -f 1`.$NET
        ADDR=2001:db8:abc:$cnt::$srv
        PORTS=`echo $HOST | cut -d ',' -f 2-`
        ...
        # link probe to switch
        PYTHONPATH=. $TIME -ao $PROBELLOG python2 topology/topology.py -a -l $NET.$port:$SRV.1
